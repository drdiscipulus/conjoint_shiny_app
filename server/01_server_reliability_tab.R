...
rv2 <- reactiveValues(dat = NULL, check = NULL, compute = NULL, reset = FALSE)
...
observeEvent(input$upload_data, {
  ...
  if (!is.null(input$upload_data)) {
    rv2$dat <- file_upload(input$upload_data)
  }
})
...
observeEvent(input$show_table, {
  ...
  if (is.null(input$upload_data)) {
    ...
    showModal(modalDialog(
      ...
      renderReactable(reactable(rv2$dat,
        highlight = TRUE,
        striped = TRUE,
        bordered = TRUE,
        compact = TRUE,
...
observeEvent(input$show_class, {
  ...
  if (is.null(input$upload_data)) {
    ...
    showModal(modalDialog(
      ...
      # Render that data as a reactable
      renderReactable(reactable(class_type_overview(rv2$dat),
        highlight = TRUE,
        striped = TRUE,
        bordered = TRUE,
        compact = TRUE,
...
observeEvent(input$check_data, {
  ...
  att_num <- attribute_checker(rv2$dat)
  ...
  round_test <- try(round_checker(rv2$dat), silent = TRUE)
...
icc_table <- reactive({
  ...
  req(rv2$dat, rv2$check, rv2$compute)
...
rel_table <- reactive({
  ...
  if (rv2$compute == "go") {
    ...
    cor_res <- rel_cor(rv2$dat)

    # Get results from icc_table
    icc_res <- icc_table()

    # Create a data frame with correlation data
    cor_res <- tibble(profile = unique(rv2$dat$profile), r = round(cor_res, 2))
...
output$violin <- renderPlotly({
  ...
  if (rv2$compute == "go") {
    ...
    df_dev <- compute_deviation(dat = rv2$dat)
    ...
    violin_plot(
      dat = df_dev,
      num_profiles = length(unique(rv2$dat$profile)),
      plot_height = 370,
